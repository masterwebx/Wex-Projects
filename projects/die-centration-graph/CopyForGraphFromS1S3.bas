Attribute VB_Name = "CopyForGraphFromS1S3"
Option Explicit

' Import into Personal.xlsb or a launcher workbook — NOT into S1 S3.xlsm.
' Self-contained: copies S1 S3.xlsm, opens the copy hidden (macros disabled),
' builds DIEGRAPH2 from that copy, then closes it. No macro inside S1 S3 is used.
'
' Button: CopyForGraphFromS1S3.CopyForGraphFromS1S3
...
Private Const SOURCE_XLSM As String = "G:\Shipping\100% Inspection Sheets\Production Folder\1 - Quality\Files\S1 S3.xlsm"
Private dataWb As Workbook

Public Sub CopyForGraphFromS1S3()
    CopyForGraphFromFile SOURCE_XLSM, "s1s3"
End Sub
...
Private Function LookupTableTsv() As String
    ...
    ' 2) Workbook S1 S3 VLOOKUPs, if it is already open
    ...
    ' 3) Same [n]Master Sheet cache S1 S3 uses, so specs match E6/G6 even if AIO is closed
