...
assert.match(hta, /ICON="QualityDesk\.ico"/);
{
  const icoPath = path.join(dir, 'QualityDesk.ico');
...
assert.match(sealedHello, /^QDSEAL2/);
...
const sealedOld = QD.seal1('legacy pack');
assert.match(sealedOld, /^QDSEAL1/);
assert.equal(QD.unseal(sealedOld), 'legacy pack');
assert.equal(QD.unseal(QD.seal('A'.repeat(10000))).length, 10000);
...
const releaseBuild = fs.readFileSync(path.join(dir, 'tools/build-release.mjs'), 'utf8');
assert.match(releaseBuild, /function bootPathRemote/);
assert.match(releaseBuild, /function bootStageLocal/);
assert.match(releaseBuild, /function bootRelaunch/);
assert.match(releaseBuild, /qd-home\.txt/);
assert.match(releaseBuild, /QD_NET_HOME/);
assert.match(releaseBuild, /qd-core-boot\.tmp/);
assert.match(releaseBuild, /function bootProfWrite/);
assert.match(releaseBuild, /profile\.txt/);
...
assert.match(hta, /\\\\aio-csv\\\\/);
assert.match(hta, /function sealLegacyPlainFiles/);
assert.match(hta, /function sealPlainFilesInFolder/);
assert.match(hta, /function fileIsSealed/);
assert.match(hta, /function peekHead/);
assert.match(hta, /deskPackCache/);
assert.match(hta, /qd-stamp\.txt/);
assert.match(hta, /aio\.loadAioCache/);
assert.match(hta, /QD_RELEASE_CRYPT/);
assert.match(html, /\\u00d7/);
assert.doesNotMatch(html, /\/\?\/g/);
assert.match(hta, /results\\\\" \+ name/);
assert.match(hta, /writeUtf8\(dst, text, true\)/);
assert.match(hta, /"index\.html"\), html, true/);
assert.match(hta, /v1\.7\.79/);
assert.match(hta, /function lastCheckPath/);
assert.match(hta, /last-check\.js/);
assert.match(hta, /function fillLastChecksLazy/);
assert.match(hta, /function loadLastChecks/);
assert.match(hta, /boot\.loadLastChecks/);
assert.match(hta, /fillLastChecksLazy\(\)/);
{
  const lastFn = hta.match(/function lastRowFor\(lineId\) \{[\s\S]*?\n        \}/);
  assert.ok(lastFn, "lastRowFor");
  assert.match(lastFn[0], /lastCheckByFile/);
  assert.doesNotMatch(lastFn[0], /readLineRows/);
}
...
assert.match(html, /loadedLineFiles/);
assert.match(html, /const KNOWN_LINES/);
assert.match(html, /function loadChartJs/);
assert.match(html, /async function ensureLookupLoaded/);
assert.match(hta, /function deskWebStamp/);
assert.doesNotMatch(hta, /function deskHtmlStamp/);
assert.match(hta, /stamp = deskWebStamp\(\)/);
assert.match(hta, /function warmDeskLineFiles/);
assert.match(hta, /function copyDeskResultFile/);
assert.match(hta, /skipMeta/);
assert.match(hta, /button:active/);
assert.match(hta, /deskReady/);
assert.match(hta, /if \(!session\.user\) return false;/);
assert.match(hta, /import\.readHandoff/);
assert.match(hta, /try \{ p = materializeDeskHtml\(\); \}/);
assert.match(html, /gcoex.*garland|key === "gcoex"/);
assert.match(hta, /function profFlush/);
...
assert.match(hta, /theme-cache\.txt/);
assert.match(hta, /edge-console\.log/);
assert.match(html, /function profFlush/);
assert.match(html, /QDPROF_LINE/);
assert.match(html, /function profSendToHta/);
assert.match(html, /html.ensurePostings/);
assert.match(hta, /profile\.txt/);
assert.match(hta, /function pathIsRemote/);
assert.match(hta, /function streamReadFile/);
assert.match(hta, /QD_NET_HOME/);
...
if (fs.existsSync(releaseHtaPath) && fs.existsSync(releaseCorePath)) {
  ...
  assert.doesNotMatch(releasePack.app, /function deskHtmlStamp/);
  assert.match(releasePack.app, /function warmDeskLineFiles/);
  assert.match(releasePack.app, /skipMeta/);
  assert.match(releasePack.web, /async function ensurePlantLoaded/);
  assert.match(releasePack.web, /const PLANT_LINE_FILES/);
  assert.match(releasePack.web, /const KNOWN_LINES/);
  assert.match(releasePack.web, /function loadChartJs/);
  assert.match(releasePack.app, /function profFlush/);
  assert.match(releasePack.app, /function pathIsRemote/);
  assert.match(releasePack.app, /QD_NET_HOME/);
  assert.match(releasePack.app, /ICON="QualityDesk\.ico"/);
  ...
  assert.doesNotMatch(releasePack.app, /\(0,eval\)\(o\)\}/);
  assert.match(releasePack.app, /loginOverlay/);
  assert.match(releasePack.web, /<!DOCTYPE html>|<html/i);
  assert.doesNotMatch(releasePack.web, /\/\?\/g/);
  assert.match(releasePack.web, /\\u00d7/);
