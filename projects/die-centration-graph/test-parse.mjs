...
assert.equal(QD.VERSION, '1.7.75');
...
assert.match(hta, /if \(!force && loadAioCache\(\)\)/);
...
if (fs.existsSync(releaseHtaPath) && fs.existsSync(releaseCorePath)) {
  ...
  assert.match(releasePack.app, /v1\.7\.75/);
  assert.match(releasePack.app, /QD\.VERSION = '1\.7\.75'/);
