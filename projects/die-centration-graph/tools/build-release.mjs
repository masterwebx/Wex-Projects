...
function booterSource(cryptJs) {
  return [
    ...
    '}',
    '',
    'function bootPathRemote(path) {',
    '    var p = String(path || ""), fso, drive;',
    '    if (p.indexOf("\\\\") === 0) return true;',
    '    if (p.length >= 2 && p.charAt(1) === ":") {',
    '        try {',
    '            fso = new ActiveXObject("Scripting.FileSystemObject");',
    '            drive = fso.GetDrive(p.substring(0, 2));',
    '            return !!(drive && drive.DriveType === 3);',
    '        } catch (eR) {}',
    '    }',
    '    return false;',
    '}',
    '',
    'function bootTempDir() {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var sh, dir = "";',
    '    try {',
    ...
    '        try { dir = fso.GetSpecialFolder(2).Path; } catch (e1) { dir = bootFolder(); }',
    '    }',
    '    return dir;',
    '}',
    '',
    'function bootLocalDir() {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var sh, root = "", dir;',
    '    try {',
    '        sh = new ActiveXObject("WScript.Shell");',
    ...
    '    } catch (e0) {}',
    '    if (!root || root.indexOf("%") >= 0) root = bootTempDir();',
    '    dir = fso.BuildPath(root, "QualityDesk");',
    '    if (!fso.FolderExists(dir)) fso.CreateFolder(dir);',
    '    return dir;',
    '}',
    '',
    'function bootCopyNewer(src, dest) {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var sm, dm;',
    '    if (!fso.FileExists(src)) return false;',
    '    if (fso.FileExists(dest)) {',
    '        sm = +new Date(fso.GetFile(src).DateLastModified);',
    '        dm = +new Date(fso.GetFile(dest).DateLastModified);',
    '        if (dm >= sm && fso.GetFile(dest).Size === fso.GetFile(src).Size) return true;',
    '        try { fso.DeleteFile(dest, true); } catch (eD) {}',
    '    }',
    '    fso.CopyFile(src, dest, true);',
    '    return fso.FileExists(dest);',
    '}',
    '',
    'function bootReadHome(dir) {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var path = fso.BuildPath(dir, "qd-home.txt"), ts, s = "";',
    ...
    '}',
    '',
    'function bootWriteHome(dir, home) {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var ts = fso.CreateTextFile(fso.BuildPath(dir, "qd-home.txt"), true);',
    '    ts.Write(String(home || ""));',
    '    ts.Close();',
    '}',
    '',
    'function bootStageLocal(netFolder) {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var local = bootLocalDir();',
    '    var names = ["QualityDesk.hta", "qd.core", "QualityDesk.ico"], i, src, dest;',
    '    for (i = 0; i < names.length; i++) {',
    '        src = fso.BuildPath(netFolder, names[i]);',
    '        dest = fso.BuildPath(local, names[i]);',
    '        bootCopyNewer(src, dest);',
    '    }',
    '    src = fso.BuildPath(netFolder, "qualitydesk.hta");',
    '    if (fso.FileExists(src) && !fso.FileExists(fso.BuildPath(local, "QualityDesk.hta"))) {',
    '        bootCopyNewer(src, fso.BuildPath(local, "QualityDesk.hta"));',
    '    }',
    '    bootWriteHome(local, netFolder);',
    '    return local;',
    '}',
    '',
    'function bootRelaunch(localHta) {',
    '    var sh = new ActiveXObject("WScript.Shell");',
    '    var windir = String(sh.ExpandEnvironmentStrings("%WINDIR%") || "C:\\\\Windows");',
    '    var mshta = windir + "\\\\System32\\\\mshta.exe";',
    '    sh.Run("cmd /c ping 127.0.0.1 -n 2 >nul & start \\"\\" \\"" + mshta + "\\" \\"" + localHta + "\\"", 0, false);',
    '    window.close();',
    '}',
    '',
    'function bootRead(path) {',
    '    var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '    var stream, text, local = path, tmp = "";',
    '    try {',
    '        if (bootPathRemote(path)) {',
    '            tmp = fso.BuildPath(bootTempDir(), "qd-core-boot.tmp");',
    '            if (fso.FileExists(tmp)) fso.DeleteFile(tmp, true);',
    '            fso.CopyFile(path, tmp, true);',
    '            local = tmp;',
    '        }',
    ...
    '        stream.Open();',
    '        stream.LoadFromFile(local);',
    '        text = stream.ReadText();',
    '        stream.Close();',
    '        return text;',
    '    } finally {',
    '        try { if (tmp && fso.FileExists(tmp)) fso.DeleteFile(tmp, true); } catch (eD) {}',
    '    }',
    '}',
    '',
    ...
    '}',
    '',
    'function bootJsEscape(s) {',
    '    return String(s == null ? "" : s).replace(/\\\\/g, "\\\\\\\\").replace(/"/g, "\\\\\\"");',
    '}',
    '',
    'function injectReleaseFlag(html, netHome) {',
    '    var tag = "<script language=\\"JScript\\">window.QD_RELEASE_CRYPT=1;";',
    '    if (netHome) tag += "window.QD_NET_HOME=\\"" + bootJsEscape(netHome) + "\\";";',
    '    tag += "<\\/script>";',
    '    var s = String(html || "");',
    ...
    '}',
    '',
    'var bootProfT0 = new Date().getTime();',
    'var bootProfLast = bootProfT0;',
    'var bootProfLines = [];',
    '',
    'function bootProf(name) {',
    '    var now = new Date().getTime();',
    '    bootProfLines.push({ name: String(name || ""), step: now - bootProfLast, total: now - bootProfT0 });',
    '    bootProfLast = now;',
    '}',
    '',
    'function bootProfWrite(kind) {',
    '    var fso, path, ts, i, r, name, s, now, net;',
    '    try {',
    '        fso = new ActiveXObject("Scripting.FileSystemObject");',
    '        path = fso.BuildPath(bootLocalDir(), "profile.txt");',
    '        s = "";',
    '        if (kind === "begin") {',
    '            now = new Date();',
    '            s += "Quality Desk profile (booter)\\r\\n";',
    '            s += "When: " + now.toDateString() + " " + now.toTimeString() + "\\r\\n";',
    '            try { net = new ActiveXObject("WScript.Network"); s += "PC: " + net.ComputerName + "  user: " + net.UserName + "\\r\\n"; } catch (eN) {}',
    '            s += "File: " + path + "\\r\\n\\r\\n--- booter ---\\r\\n";',
    ...
    '            s = "";',
    '        }',
    '        for (i = 0; i < bootProfLines.length; i++) {',
    '            r = bootProfLines[i];',
    '            name = r.name;',
    '            while (name.length < 46) name += " ";',
    '            s += r.step + " ms  tot " + r.total + " ms  " + name + "\\r\\n";',
    '        }',
    '        bootProfLines = [];',
    '        if (!s) return;',
    ...
    '    try {',
    '        var fso = new ActiveXObject("Scripting.FileSystemObject");',
    '        var folder, path, raw, pack, html, home, local, localHta;',
    '        bootProfWrite("begin");',
    '        folder = bootFolder();',
    '        bootProf("bootFolder " + folder);',
    '        if (bootPathRemote(folder)) {',
    '            bootProf("remote yes");',
    '            bootFail("Network share is locked down. Opening a local launcher…");',
    '            local = bootStageLocal(folder);',
    '            bootProf("stageLocal");',
    '            localHta = fso.BuildPath(local, "QualityDesk.hta");',
    '            if (!fso.FileExists(localHta) || !fso.FileExists(fso.BuildPath(local, QD.CORE_FILE))) {',
    '                bootFail("Could not copy QualityDesk.hta and qd.core to this PC from\\n" + folder);',
    '                bootProfWrite();',
    '                return;',
    '            }',
    '            bootProfWrite();',
    '            bootRelaunch(localHta);',
    '            return;',
    '        }',
    '        bootProf("remote no");',
    '        home = bootReadHome(folder);',
    '        bootProf("readHome " + (home || "(none)"));',
    '        if (home) {',
    '            try { if (fso.FolderExists(home)) bootStageLocal(home); } catch (eSt) {}',
    '            bootProf("refreshFromShare");',
    '        }',
    '        path = fso.BuildPath(folder, QD.CORE_FILE);',
    ...
    '        }',
    '        raw = bootRead(path);',
    '        bootProf("read qd.core " + (raw ? raw.length : 0) + " chars");',
    '        if (QD.isSealed(raw)) raw = QD.unseal(raw);',
    '        bootProf("unseal qd.core");',
    ...
    '            return;',
    '        }',
    '        html = injectReleaseFlag(stripHtaApp(pack.app), home);',
    '        bootProf("inject html " + html.length + " chars");',
    '        bootProfWrite();',
    '        document.open();',
    '        document.write(html);',
    '        document.close();',
    '    } catch (eBoot) {',
    '        try { bootProf("ERROR " + (eBoot && eBoot.message ? eBoot.message : eBoot)); bootProfWrite(); } catch (eP) {}',
